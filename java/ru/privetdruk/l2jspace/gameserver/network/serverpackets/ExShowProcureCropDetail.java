...
public class ExShowProcureCropDetail extends GameServerPacket {
    ...
    private final Map<Integer, CastleManorManager.CropProcure> _castleCrops;

    public ExShowProcureCropDetail(int cropId) {
        _cropId = cropId;
        _castleCrops = new HashMap<>();
        for (Castle c : CastleManager.getInstance().getCastles()) {
            final CastleManorManager.CropProcure cropItem = c.getCrop(_cropId, CastleManorManager.PERIOD_CURRENT);
    ...
    public void writeImpl() {
        ...
        for (Entry<Integer, CastleManorManager.CropProcure> entry : _castleCrops.entrySet()) {
            final CastleManorManager.CropProcure crop = entry.getValue();
