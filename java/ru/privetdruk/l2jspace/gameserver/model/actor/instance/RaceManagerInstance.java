...
public class RaceManagerInstance extends FolkInstance {
    ...
    public void onBypassFeedback(PlayerInstance player, String command) {
        if (command.startsWith("BuyTicket")) {
            ...
            final MonsterRace.HistoryInfo info = MonsterRace.getInstance().getHistory().get(raceId - 1);
            if (info == null) {
                super.onBypassFeedback(player, "Chat 0");
                return;
            }
            ...
            final List<MonsterRace.HistoryInfo> history = MonsterRace.getInstance().getHistory();
            for (int i = history.size() - 1; i >= Math.max(0, history.size() - 7); i--) {
                final MonsterRace.HistoryInfo info = history.get(i);
