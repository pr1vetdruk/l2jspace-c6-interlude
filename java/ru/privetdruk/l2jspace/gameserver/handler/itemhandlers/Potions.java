...
public class Potions implements IItemHandler {
    ...
    public synchronized void useItem(Playable playable, ItemInstance item) {
        if (playable instanceof PlayerInstance) {
            ...
            switch (itemId) {
                ...
                {
                    if (!isEffectReplaceable(playable, Effect.EffectType.MANA_HEAL_OVER_TIME, itemId)) {
                        return;
                    }
                ...
                {
                    if (!isEffectReplaceable(playable, Effect.EffectType.HEAL_OVER_TIME, itemId)) {
                        return;
                    }
                ...
                case 8627: {
                    // elixir of Life
                    if (!isEffectReplaceable(activeChar, Effect.EffectType.HEAL_OVER_TIME, itemId)) {
                        return;
                    }
    ...
    private boolean isEffectReplaceable(Playable activeChar, Enum<Effect.EffectType> effectType, int itemId) {
